...
public class AddMenuOperation : AddOperation<MenuBar, MenuBarItem>
{
    ...
    public AddMenuOperation( Design design, string? name, string? menuItemText = ViewFactory.DefaultMenuItemText )
        : base(
            ( v ) => v.Menus,
            ( v, a ) => v.Menus = a,
            ( s ) => s.Title.ToString( ) ?? "blank menu",
            ( v, n ) => { return new MenuBarItem( n, new MenuItem[] { new MenuItem { Title = menuItemText } } ); },
            design,
            name )
    {
    }
